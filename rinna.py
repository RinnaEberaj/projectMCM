...
df["video_views_for_the_last_30_days"] = (
    df["video_views_for_the_last_30_days"] / 1000000
)
...
df.rename(
    columns={
        "subscribers": "subscribers_per_millon",
        ...
        "video views": "video_views_per_thousands_millions",
    },
    inplace=True,
)

df_views_category = (
    df[["category", "video_views_per_thousands_millions"]].groupby(by="category").sum()
)
df_views_category = df_views_category.sort_values(
    by=["video_views_per_thousands_millions"], ascending=False
)
...
fig = px.bar(
    ...
    labels={
        "video_views_per_thousands_millions": "Video views",
        "category": "Category",
    },
)
...
# Define df_views30 globally
df_views30 = pd.DataFrame()


# Create a callback to update the second graph based on the selected dropdown value
@app.callback(
    Output("graph-output-2", "figure"), Input("top-youtuber-dropdown", "value")
)
def update_top_youtuber_graph(selected_value):
    global df_views30  # Use the global df_views30 variable
    ...
    df_views30.loc[:, "Rank"] = df_views30["video_views_last_30_days_per_millon"].rank(
        ascending=False
    )
    df_views30 = df_views30[df_views30["Rank"] <= 10].sort_values(by=["Rank"])

    df_filtered = df_views30[df_views30["Rank"] <= selected_value].sort_values(
        by=["Rank"]
    )
    ...
    fig2 = px.bar(
        ...
        color_discrete_sequence=["red", "white"],  # You can customize colors here
        labels={
            "video_views_last_30_days_per_millon": "Video views",
            "Youtuber": "Youtuber",
        },
    )
    ...
    [
        ...
        dcc.Tabs(
            [
                ...
                dcc.Tab(
                    label="Category vs Video Views",
                    children=[
                        dcc.Graph(
                            id="graph-output", figure=fig
                        ),  # First graph with 'graph-output' ID
                    ],
                ),
                # Tab for the second graph
                # Tab for the second graph
                dcc.Tab(
                    label="Top YouTuber's Video Views",
                    children=[
                        html.Br(),
                        ...
                        dcc.Graph(id="graph-output-2"),  # Add this line
                    ],
                ),
                # Tab for the pie chart or top 3 countries pie chart
                dcc.Tab(
                    label="Countries with Most YouTubers",
                    children=[
                        html.Div(
                            [
                                html.Br(),
                                ...
                                ),  # Empty graph for initial display
                            ]
                        )
                    ],
                ),
...
@app.callback(Output("pie-chart", "figure"), Input("pie-chart-selector", "value"))
